...
export class DomainImplement<T extends JSONPrimitives> implements Domain<T> {
    ...
    copy(): Domain<T> {
        return new DomainImplement<T>(this.toJSON())
    }
    ...
    static fromJSON<T extends JSONPrimitives>(json: JSONArray): Domain<T> {
        if (DomainImplement.validateJSON(json)) {
            return new DomainImplement<T>(json as T[])
        }
